...
export const ContactsManager: React.FC<ContactsManagerProps> = ({ onClose, onSelectContact }) => {
  ...
  const refresh = () => {
    const results = searchContacts(query, filter, sortBy, sortDir, groupFilter || undefined);
    setContacts(results);
    setGroups(getGroups());
  };

  useEffect(() => { refresh(); }, [query, filter, sortBy, sortDir, groupFilter]);
  ...
  const handleImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      ...
      const result = importContacts(text);
      showToast(`Imported ${result.imported} contacts${result.errors ? `, ${result.errors} skipped` : ''}`);
      refresh();
    };
  ...
  const handleShare = async () => {
    ...
    if (allContacts.length === 0) {
      showToast('No contacts to share');
      return;
    }
    const text = shareContactsAsText(allContacts);
    try {
      if (navigator.share) {
        await navigator.share({ title: 'CeloFlow Contacts', text });
      } else {
      ...
      <div className="w-full max-w-2xl max-h-[90vh] bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 flex flex-col overflow-hidden animate-fade-in-up">
        ...
        <div className="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-bold text-gray-900 dark:text-white text-lg">Contacts</h2>
              <p className="text-xs text-gray-500 dark:text-gray-400">{contacts.length} contact{contacts.length !== 1 ? 's' : ''}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={() => { setShowForm(true); setEditingId(null); setForm(EMPTY_FORM); setFormErrors({}); }}
              className="p-2 bg-celo-green text-white rounded-lg hover:bg-green-500 transition-colors" title="Add Contact">
        ...
        <div className="px-6 py-3 border-b border-gray-100 dark:border-gray-700 flex flex-wrap gap-2 items-center shrink-0">
          ...
          <div className="relative">
            <button onClick={() => { setShowFilterMenu(!showFilterMenu); setShowSortMenu(false); }}
              ...
              <Filter className="w-3 h-3" /> {filter === 'all' ? 'All' : filter === 'favorites' ? 'Favorites' : 'Blocked'}
          ...
          <div className="flex gap-1 ml-auto">
            <button onClick={handleExport} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors" title="Export">
              <Download className="w-4 h-4 text-gray-500" />
            </button>
            <button onClick={() => fileInputRef.current?.click()} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors" title="Import">
              <Upload className="w-4 h-4 text-gray-500" />
            </button>
            <button onClick={handleShare} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors" title="Share">
        ...
        <div className="flex-1 overflow-y-auto">
          {contacts.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-gray-400">
              ...
              <p className="text-sm font-medium">{query ? 'No matching contacts' : 'No contacts yet'}</p>
                ...
                <div key={contact.id}
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                    <button onClick={() => handleCopyAddress(contact.address)}
                      className="p-1.5 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-lg transition-colors" title="Copy address">
                      <Copy className="w-3.5 h-3.5 text-gray-400" />
                    </button>
                    <button onClick={() => handleToggleFavorite(contact.id)}
                      className="p-1.5 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-lg transition-colors" title="Toggle favorite">
                    ...
                    <button onClick={() => handleToggleBlocked(contact.id)}
                      className="p-1.5 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-lg transition-colors" title="Toggle blocked">
                      <Ban className={`w-3.5 h-3.5 ${contact.blocked ? 'text-red-500' : 'text-gray-400'}`} />
                    </button>
                    <button onClick={() => handleEdit(contact)}
                      className="p-1.5 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-lg transition-colors" title="Edit">
                      <Edit3 className="w-3.5 h-3.5 text-gray-400" />
                    </button>
                    <button onClick={() => setConfirmDelete(contact.id)}
                      className="p-1.5 hover:bg-red-100 dark:hover:bg-red-900/30 rounded-lg transition-colors" title="Delete">
          ...
          <div className="absolute inset-0 bg-black/30 flex items-center justify-center z-50 rounded-2xl">
            <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-xl border border-gray-200 dark:border-gray-700 max-w-sm mx-4">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h3 className="font-bold text-gray-900 dark:text-white">Delete Contact</h3>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="col-span-2">
                  <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Wallet Address *</label>
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Network</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1 flex items-center gap-1"><Phone className="w-3 h-3" /> Phone</label>
                  <input type="tel" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })}
                    className="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 text-gray-900 dark:text-white rounded-lg text-sm outline-none focus:ring-2 focus:ring-celo-green"
                    placeholder="+1 234 567 8900" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1 flex items-center gap-1"><Mail className="w-3 h-3" /> Email</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1 flex items-center gap-1"><MapPin className="w-3 h-3" /> City</label>
                  <input type="text" value={form.city} onChange={(e) => setForm({ ...form, city: e.target.value })}
                    className="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 text-gray-900 dark:text-white rounded-lg text-sm outline-none focus:ring-2 focus:ring-celo-green"
                    placeholder="Manila" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1 flex items-center gap-1"><Globe className="w-3 h-3" /> Country</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Avatar URL</label>
                  <input type="url" value={form.avatar} onChange={(e) => setForm({ ...form, avatar: e.target.value })}
                    className="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 text-gray-900 dark:text-white rounded-lg text-sm outline-none focus:ring-2 focus:ring-celo-green"
                    placeholder="https://..." />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Group</label>
                  <input type="text" value={form.group} onChange={(e) => setForm({ ...form, group: e.target.value })}
                    className="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 text-gray-900 dark:text-white rounded-lg text-sm outline-none focus:ring-2 focus:ring-celo-green"
                    placeholder="Family, Work, etc." list="group-suggestions" />
              ...
              {/* Notes */}
              <div>
                <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1 flex items-center gap-1"><FileText className="w-3 h-3" /> Notes</label>
